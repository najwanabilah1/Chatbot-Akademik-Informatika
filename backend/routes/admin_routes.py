...
import os
import pytz; WIB = pytz.timezone("Asia/Jakarta")
        ...
        recent_chat_data = [
            {
                ...
                "last_chat": r.last_chat.astimezone(WIB).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if os.path.isdir(dataset_dir):
            for f in sorted(os.listdir(dataset_dir), key=lambda x: os.path.getmtime(os.path.join(dataset_dir, x)), reverse=True):
                ...
                try:
                    ...
                    files.append({
                        "filename": f,
                        "uploaded_at": datetime.fromtimestamp(stat.st_mtime, WIB).strftime("%Y-%m-%d %H:%M:%S"),
                        "size": stat.st_size
                    })
                except Exception:
                    continue
...
@admin_bp.route("/uploads/peraturan", methods=["GET"])
def list_uploaded_peraturan():
    try:
            ...
            if os.path.isdir(pdf_dir):
                for f in sorted(os.listdir(pdf_dir), key=lambda x: os.path.getmtime(os.path.join(pdf_dir, x)), reverse=True):
                    ...
                    try:
                        ...
                        continue
        if file_type in ["word", "all"]:
            if os.path.isdir(word_dir):
                for f in sorted(os.listdir(word_dir), key=lambda x: os.path.getmtime(os.path.join(word_dir, x)), reverse=True):
                    if not (f.lower().endswith(".doc") or f.lower().endswith(".docx")):
                        continue
        ...
        print("❌ Error listing peraturan uploads:", e)
        return jsonify({"error": str(e)}), 500
